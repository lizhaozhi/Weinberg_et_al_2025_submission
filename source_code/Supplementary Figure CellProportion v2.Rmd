...
data <- read_excel(data_path)
csf_data <- data %>%
  filter(tissue == "csf", treatment == "Ctrl")
...
# Average across donors per month
grouped <- csf_data %>%
...
df_long <- grouped %>%
  pivot_longer(cols = all_of(cell_types), names_to = "cell_type", values_to = "proportion") %>%
  mutate(cell_label = recode(cell_type, !!!cell_map),
         month = factor(as.character(month), levels = c("0", "3", "12")))

# Order by median baseline (month 0) proportion
cell_order <- df_long %>%
  filter(month == "0") %>%
  group_by(cell_label) %>%
  summarise(median_prop = median(proportion, na.rm = TRUE), .groups = "drop") %>%
  arrange(desc(median_prop)) %>%
  pull(cell_label)

df_long$cell_label <- factor(df_long$cell_label, levels = cell_order)

# Define manual significance annotations
brackets <- data.frame(
  cell_label = c("B", "NK", "NK"),
  Month = c("3", "3", "12"),
  label = c("*", "*", "*"),
  x_start = c(7.75, 6.75, 6.75),
  x_end = c(8, 7, 7.3),
  y = c(0.12, 0.10, 0.15)
)

# Plot
p_csf_time = ggplot(df_long, aes(x = cell_label, y = proportion, fill = month)) +
  geom_boxplot(outlier.shape = NA, width = 0.6, position = position_dodge(width = 0.75)) +
  geom_jitter(aes(color = month), size = 1.2, shape = 21,
              position = position_jitterdodge(jitter.width = 0.25, dodge.width = 0.75), alpha = 0.7,
              show.legend = FALSE) +
  ...
  geom_text(data = brackets,
            aes(x = (x_start + x_end)/2, y = y + 0.01, label = label),
            inherit.aes = FALSE, size = 5) +
  scale_fill_manual(values = c("#C7DB9C", "#FBDB93", "#f8cbc8")) +
  scale_color_manual(values = c("#67AE6E", "#EF9651", "#dd6b70")) +
  #scale_color_manual(values = rep("black", 3)) +
  labs(x = "Cell Type", y = "Cell Proportion", title = "CSF Cell Proportions Over Time", fill = "Month") +
...
```{r}
ggarrange(
  p_baseline_celltype + theme(plot.title = element_blank()),
  p_bar_ctrl,
  p_baseline_ad,
  p_csf_time + theme(plot.title = element_blank()),
  ncol = 2, nrow = 2
)
ggsave("../outputs/Supplementary Figure CellProportion.tiff", height = 6, width = 10)
```
